...
В проекте есть отдельный модуль `parser` для автоматического сбора и преобразования результатов с сайта `velomarathon.ru` (и вложенных `timingband.ru`, `sportident.online` и др.) в единый формат.
1. Убедитесь, что backend-сервер запущен.
2. Перейдите в корень проекта. Установите зависимости парсера, если еще не сделали этого:
   ```bash
   pip install -r parser/requirements.txt
   ```
3. Запустите скрипт парсинга:
   ```bash
   PYTHONPATH=. python parser/main.py
   ```
