...
class SportidentParser(BaseParser):
    @classmethod
    def can_handle_url(cls, url: str) -> bool:
        return 'sportident.online' in url.lower()

    def parse(self, url: str, page: Page) -> List[Result]:
        print(f"SportidentParser: Scraping race {url}")
        try:
        ...
        for i, th in enumerate(headers):
            text = th.text.lower()
            if "имя" in text or "name" in text or "участник" in text or "спортсмен" in text or "фамилия, имя" in text:
                name_idx = i
