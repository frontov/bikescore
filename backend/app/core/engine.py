...
def process_race(session: Session, race_id: int):
    ...
    for r in valid_results:
        groups[(r.rider.gender, r.rider.is_kids)].append(r)

    k_factor = race.k_factor
    category = race.category # 'road', 'gravel', 'mtb'

    for (gender, is_kids), group_results in groups.items():
        if not group_results:
            continue

        # Sort group by time to assign places
        group_results.sort(key=lambda r: r.time_sec)

        # Assign places independently for each group
        for idx, r in enumerate(group_results):
            r.place = idx + 1

        # Find winner time for the group
        winner_time = group_results[0].time_sec

        # Filter outliers (>30% of winner time)
        # i.e., time_sec <= winner_time * 1.3
        filtered_results = [r for r in group_results if r.time_sec <= winner_time * 1.3]

        N = len(filtered_results)
        if N < 2:
            # No one to compare with in this group, just save 0 delta if needed
